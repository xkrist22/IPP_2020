 ...
 * @param string $parse_script contains path to file containing file with parser.php
 ...
 */
function check_app($dir_with_tests, $parse_script, $int_script, $recursive, $work_dir, $is_recursive_call = false) {
    $html = html_gen::get_instance("parse.php and interpret.py");
    ...
    foreach ($extracted_test_files as $test_file) {
        ...
        if (is_dir( $work_dir . $test_file) && $recursive) {
            check_app($dir_with_tests . DIRECTORY_SEPARATOR . $test_file, $parse_script, $int_script, $recursive, $work_dir . $test_file . DIRECTORY_SEPARATOR, true);
        } else {
            ...
            if ($generated_return_code == 0) {
                ...
                exec("python3 " . $int_script . " --source=" . $temp_xml_file . " --input=" . $input_file . " >" . $result_file, $_, $generated_return_code);
                ...
                if (($declared_return_code == $generated_return_code && $declared_return_code != 0 && $generated_return_code != 0)) {
                    $html->insert_passed($work_dir . $test_file_info['filename'], $declared_return_code, $generated_return_code);
                } else {
                    $html->insert_failed($work_dir . $test_file_info['filename'], $declared_return_code, $generated_return_code);
                }
